...
rep=call("report/exec_report",{"reportResourceId":RES,"reportTemplateId":13,"reportObjectId":GRP,"reportObjectSecId":0,"interval":{"from":st,"to":en,"flags":0}},sid)
hdr=rep["reportResult"]["tables"][0].get("header",[])
tg("КОЛОНКИ:\n"+"\n".join(f"col{i}: {h}" for i,h in enumerate(hdr)))
rows=call("report/select_result_rows",{"tableIndex":0,"config":{"type":"range","data":{"from":0,"to":3000,"level":1,"unitInfo":1}}},sid)
out=["СТРОКИ ур.1 (с машинами):"]; sh=0
for r in rows if isinstance(rows,list) else []:
    c=[cl(x) for x in r.get("c",[])]
    if not c or "----" in c[0]: continue
    out.append("• "+" ║ ".join(f"[{i}]{v}" for i,v in enumerate(c))[:340]); sh+=1
    if sh>=6: break
tg("\n".join(out))
