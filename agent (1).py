...
def normalize_code(s):
    ...
    s = re.sub(r'\(.*?\)', '', s)  # убрать (арена),(карьер) и т.п.
    ...
    for c in cands:
        if sum(ch.isdigit() for ch in c) >= 2 and len(c) >= 4 and len(c) > len(best):
            best = c
    if best:
        # опечатка 623FJ02 -> 623FI02
        if best == "623FJ02": best = "623FI02"
        return best
    for c in cands:
...
def get_wialon_fuel(sid, ts_from, ts_to):
    ...
    for idx,t in enumerate(tables):
        lbl = t.get("label","")
        rows = wcall("report/select_result_rows", {"tableIndex":idx,
            "config":{"type":"range","data":{"from":0,"to":5000,"level":2,"unitInfo":1}}}, sid)
        ...
        for r in rows:
            ...
            g = str(cells[0]).strip()
            # строка-дата?
            dm = re.match(r'^(\d{2}\.\d{2}\.\d{4})$', g)
            if dm:
                try: cur_date = datetime.strptime(g,"%d.%m.%Y")
                except: cur_date=None
                continue
            if not is_car(g): continue
            code = normalize_code(g)
            if "аправк" in lbl:
                # col1=Время col4=Заправлено
                ...
                # Сливы: col1=Начало col2=Время col5=Слито
    ...
    # дедупликация (машина+время+литры)
    for k in res:
        seen=set(); uniq=[]
        for x in res[k]:
