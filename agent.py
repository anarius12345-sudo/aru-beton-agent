...
def build_pdf(path, period, stats, pairs, B, C, sg, wl, drains):
    try:
        ...
        def nf(v,dec=1):
            ...
            return s
        def mk(rows,widths,head=BLUE):
            body=[[Paragraph(str(v),cellb if i==0 else cell) for v in r] for i,r in enumerate(rows)]
        ...
        el.append(mk(arows,[8,20,17,22,43,17,17,15,14]))
        ...
        el.append(mk(mrows,[8,26,24,34,38,40]))
        ...
        el.append(mk(brows,[8,24,22,48,18,26,24],head=RED))
        ...
        el.append(mk(crows,[8,24,22,24,22,70],head=RED))
        ...
        if drains:
            ...
            el.append(mk(drows,[8,24,22,24,22,70],head=RED))
