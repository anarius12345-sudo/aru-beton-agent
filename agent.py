...
def match_abc(sg, wl_fuel):
    """Сводим обе стороны к (машина, день) и сравниваем суммы. A — есть и карта и ДУТ; B — только карта; C — только ДУТ."""
    def agg(items):
        d={}
        for x in items:
            day = x["dt"].date() if x.get("dt") else None
            k=(x["code"], day)
            if k not in d:
                d[k]={"code":x["code"],"day":day,"liters":0.0,"dt":x.get("dt"),
