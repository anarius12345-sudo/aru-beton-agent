...
def build_excel(path, period, pairs, B, C, sg, wl, drains):
    ...
    ws["A1"]="БАЛАНС ТОПЛИВА: карты (МП) ↔ датчики (ДУТ)"; ws["A1"].font=Font(bold=True,size=13)
    ...
    sumB=sum(b.get("order_amt",0) for b in B)
    diffA=mpA-dutA
    pct=diffA/mpA*100 if mpA else 0
    diff_money=round(diffA*PRICE_DT)
    # ---- основная таблица ----
    hdr=["№","Категория","Машин","МП, л","ДУТ, л","Разница МП−ДУТ, л","Сумма / разница, ₸"]
    ...
    rows=[
        ...
        ["","ИТОГО",len(pairs)+len(B)+len(C),round(mpA+mpB,1),round(dutA+dutC,1),round(diffA,1),round(sum_all,0)],
    ]
    for r in rows: ws.append(r)
    zebra(ws,hr+1,ws.max_row,len(hdr))
    for c in range(1,len(hdr)+1): ws.cell(row=ws.max_row,column=c).font=Font(bold=True)  # жирный ИТОГО
    # ---- проверка сходимости (отдельная таблица) ----
    ws.append([])
    ws.append(["ПРОВЕРКА СХОДИМОСТИ БАЛАНСОВ"]); ws.cell(row=ws.max_row,column=1).font=Font(bold=True,size=11)
    hdr2=["№","Показатель","Значение, л","Контроль","Статус"]
    ws.append(hdr2); hr2=ws.max_row; style_head(ws,hr2,len(hdr2))
    chk=[
    ...
    for r in chk: ws.append(r)
    zebra(ws,hr2+1,ws.max_row,len(hdr2))
    for col,w in zip("ABCDEFG",[5,38,14,16,16,18,18]): ws.column_dimensions[col].width=w

    # ---- Заправки с ДУТ (категория A) ----
    ws=wb.create_sheet("Заправки с ДУТ")
    ws["A1"]="СВЕРКА ПО МАШИНАМ ЗА ДЕНЬ (категория A)"; ws["A1"].font=Font(bold=True,size=12)
    ws["A2"]=f"Период проверки: {period}"
    ...
    # единый шрифт Arial по всем листам (как в эталонном файле)
