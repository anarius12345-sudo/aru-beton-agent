...
def get_smartgas(d_from, d_to):
    ...
    try:
        r = requests.get(f"{SMARTGAS_BASE}/transactions",
                         ...
                         params={"dateFrom": d_from.isoformat(), "dateTo": d_to.isoformat()}, timeout=90)
        ...
        for t in txns:
            ...
            out.append({
                ...
                "azs": t.get("store_title",""),
                "by_name": not bool(re.search(r'\d{3}', normalize_code(dn))),  # запись по имени без кода
            })
    except Exception as e:
        tg(f"⚠️ Smartgas ошибка: {e}")
    return out
...
def get_wialon_fuel(sid, ts_from, ts_to):
    ...
    for idx,t in enumerate(tables):
        ...
        top = wcall("report/get_result_rows", {"tableIndex":idx,"indexFrom":0,"indexTo":200}, sid)
        ...
        for di,drow in enumerate(top):
            ...
            nkids=drow.get("rows",0)
            if nkids<=0: continue
            # раскрыть детей этой строки-даты
            kids = wcall("report/select_result_rows", {"tableIndex":idx,
                "config":{"type":"rows","data":{"rows":[di],"level":1}}}, sid)
            if not isinstance(kids, list):
                kids = wcall("report/select_result_rows", {"tableIndex":idx,
                    "config":{"type":"row","data":{"row":di,"level":1}}}, sid)
...
def match_abc(sg, wl_fuel):
    ...
    for i,c in enumerate(cards):
        ...
        for j,b in enumerate(baks):
            if b["dt"] is None or c["code"]!=b["code"]: continue
...
def run_period(label, ts_from, ts_to, d_from, d_to, sid, tag):
    ...
    fuelings = wl["fuel"]; drains = wl["drain"]
...
def main():
    ...
    try: run_period("С НАЧАЛА МЕСЯЦА", int(ms.timestamp()), int(now.timestamp()), ms.date(), now.date(), sid, "месяц")
