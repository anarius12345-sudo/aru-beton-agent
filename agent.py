...
def get_wialon_fuel(sid, ts_from, ts_to):
    ...
    for idx,t in enumerate(tables):
        ...
        for di,drow in enumerate(top):
            ...
            # ВРЕМЕННАЯ ДИАГНОСТИКА: один раз дампим сырую структуру строк-машин,
            # чтобы понять, есть ли вложенные заправки (3-й уровень). Убрать после починки.
            if os.environ.get("DEBUG_WIALON") and "аправк" in lbl and di==0:
                try:
                    tg("[RAW sub keys] "+json.dumps([sorted(r.keys()) for r in sub[:4]], ensure_ascii=False)[:600])
                    tg("[RAW sub rows] "+json.dumps(sub[:3], ensure_ascii=False)[:2500])
                    nested=wcall("report/get_result_subrows",{"tableIndex":idx,"rowIndex":0,"indexFrom":0,"indexTo":50},sid)
                    tg("[RAW nested r0] "+json.dumps(nested, ensure_ascii=False)[:2000])
                except Exception as _e:
                    tg(f"[RAW dump err] {_e}")
            for k in sub:
