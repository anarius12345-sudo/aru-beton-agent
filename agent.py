        ...
        for di,drow in enumerate(top):
            ...
            if not _RAW_DONE and "аправк" in lbl:
                ...
                try:
                    ...
                    tg(f"[RAW ov err] {_e}")
            # 2) дети первой даты с d>=2 — смотрим, заправки это (d=0) или машины (d>0); и спускаемся в машину
            if "child" not in _RAW_DONE and "аправк" in lbl and drow.get("d",0)>=2 \
               and "|" not in str(([cval(c) for c in drow.get("c",[])]+[""])[0]):
                ...
                try:
                    ...
                    for j,r in enumerate(sub):
                        if (r.get("d",0) or 0)>=2:
                            deep=wcall("report/get_result_subrows",{"tableIndex":idx,"rowIndex":j,
                                "indexFrom":0,"indexTo":50}, sid)
                            nm=str(([cval(c) for c in r.get("c",[])]+[""])[0])[:14]
                            tg(f"[RAW deep j={j} ({nm})] "+json.dumps(deep[:10], ensure_ascii=False)[:2400])
                            break
                except Exception as _e:
...
def main():
    ...
    try:
        ...
        run_period("ПРОВЕРКА 05.06 (эталон A=16)", f0, t0, d0, d0, sid, "проверка0506")
