        ...
        try:
            ...
            for t in txns:
                ...
                out.append({"code":normalize_code(dn),"raw":dn,
                    "dt":(lambda d: d+timedelta(hours=5) if d else None)(parse_dt(t.get("created_at",""))),
...
def match_abc(sg, wl_fuel):
    ...
    for i,c in enumerate(cards):
        ...
        for j,b in enumerate(baks):
            ...
            # приоритет: сначала близкие по литрам, потом по времени
...
def run_period(label, ts_from, ts_to, d_from, d_to, sid, tag):
    ...
    # фильтр по локальной дате периода (Smartgas может вернуть шире из-за UTC)
    def _in_period(t):
