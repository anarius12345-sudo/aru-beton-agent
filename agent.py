        ...
        for di,drow in enumerate(top):
            ...
            if not _RAW_DONE and "аправк" in lbl:
                ...
                try:
                    ...
                    tg(f"[RAW ov err] {_e}")
            # 2) дети первой машины с d>=2 (две+ заправки) — где лежат реальные заправки
            if "child2" not in _RAW_DONE and "аправк" in lbl and drow.get("d",0)>=2 \
               and "|" in str(([cval(c) for c in drow.get("c",[])]+[""])[0]):
                _RAW_DONE.append("child2")
                try:
                    nm=str(([cval(c) for c in drow.get("c",[])]+[""])[0])[:16]
                    tg(f"[RAW child of di={di} ({nm}), d={drow.get('d')}] "
                       +json.dumps(sub[:20], ensure_ascii=False)[:3200])
                except Exception as _e:
