...
def get_wialon_fuel(sid, ts_from, ts_to):
    ...
    for idx,t in enumerate(tables):
        ...
        for di,drow in enumerate(top):
            ...
            # ВРЕМЕННАЯ ДИАГНОСТИКА (без переменных): один раз шлём сырую структуру Wialon,
            # чтобы понять, где лежат отдельные заправки. Убрать после починки чтения.
            if not _RAW_DONE and "аправк" in lbl:
                _RAW_DONE.append(1)
                try:
                    tg("[RAW top] "+json.dumps(drow, ensure_ascii=False)[:1200])
                    tg("[RAW keys] "+json.dumps([list(r.keys()) for r in sub[:8]], ensure_ascii=False)[:800])
                    tg("[RAW rows] "+json.dumps(sub[:8], ensure_ascii=False)[:3200])
                    # пробуем спуститься на уровень глубже у первой строки-машины
                    nested=wcall("report/get_result_subrows",{"tableIndex":idx,"rowIndex":di,
                        "indexFrom":0,"indexTo":50,"level":1}, sid)
                    tg("[RAW nested] "+json.dumps(nested, ensure_ascii=False)[:2500])
