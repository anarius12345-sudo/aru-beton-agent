...
def get_wialon_fuel(sid, ts_from, ts_to):
    ...
    for idx,t in enumerate(tables):
        ...
        if not isinstance(top,list): continue
        for di,drow in enumerate(top):
            dcells=[cval(c) for c in drow.get("c",[])]
            cur_date=None
            if dcells:
                dm=re.match(r'^(\d{2}\.\d{2}\.\d{4})$', str(dcells[0]).strip())
                if dm:
                    try: cur_date=datetime.strptime(dcells[0].strip(),"%d.%m.%Y")
                    except: cur_date=None
            # получить детей строки-даты
            ...
            # ВРЕМЕННАЯ ДИАГНОСТИКА (без переменных). Убрать после починки чтения.
            # 1) обзор всех строк уровня top: n / d(детей) / имя / время / литры
            if not _RAW_DONE and "аправк" in lbl:
                _RAW_DONE.append("top")
                try:
                    ov=[]
                    for r in top:
                        cc=[cval(c) for c in r.get("c",[])]+["","","","",""]
                        ov.append([r.get("n"), r.get("d"), str(cc[0])[:16], str(cc[1])[:19], str(cc[4])[:10]])
                    tg("[RAW top overview n/d/name/time/liters]\n"+json.dumps(ov[:40], ensure_ascii=False)[:3500])
                except Exception as _e:
                    tg(f"[RAW ov err] {_e}")
            # 2) дети строки-ДАТЫ (не ИТОГО): машины это (d>0) или сразу заправки (d=0); спускаемся в машину
            _isdate = bool(re.match(r'^\d{2}\.\d{2}\.\d{4}$', str(([cval(c) for c in drow.get("c",[])]+[""])[0]).strip()))
            if "child" not in _RAW_DONE and "аправк" in lbl and _isdate and drow.get("d",0)>=5:
                _RAW_DONE.append("child")
                try:
                    def _v(r):
                        cc=[cval(c) for c in r.get("c",[])]+["","","","",""]
                        return [r.get("n"), r.get("d"), str(cc[0])[:14], str(cc[1])[:19], str(cc[4])[:9]]
                    tg(f"[RAW date '{dcells[0]}' children n/d/name/time/liters]\n"
                       +json.dumps([_v(r) for r in sub[:25]], ensure_ascii=False)[:3300])
                    # спускаемся в первую машину (берём с d>=2, иначе j=0) и пробуем прочитать её заправки
                    target=next((j for j,r in enumerate(sub) if (r.get("d",0) or 0)>=2), 0)
                    nm=str(([cval(c) for c in sub[target].get("c",[])]+[""])[0])[:14]
                    deep=wcall("report/get_result_subrows",{"tableIndex":idx,"rowIndex":target,
                        "indexFrom":0,"indexTo":50}, sid)
                    tg(f"[RAW deep rowIndex={target} ({nm}), d={sub[target].get('d')}]\n"
                       +json.dumps(deep[:10], ensure_ascii=False)[:2600])
                except Exception as _e:
                    tg(f"[RAW child err] {_e}")
            for k in sub:
                cells=[cval(c) for c in k.get("c",[])]
                if not cells: continue
                g=str(cells[0]).strip()
                if not is_car(g): continue
                ...
                if "аправк" in lbl:
                    ...
                    if 20<=liters<=350:
                        res["fuel"].append({"code":code,"raw":g,"dt":tm,"liters":liters,
                    ...
                    if tm: tm=tm+timedelta(hours=5)
                    if liters>=1:
                        res["drain"].append({"code":code,"raw":g,"dt":tm,"liters":liters,
